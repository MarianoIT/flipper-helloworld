...
typedef struct {
    Gui* gui;
    ViewPort* view_port;
    FuriMessageQueue* event_queue; //Se agrega una cola de mensajes para poder recibir eventos del sistema
} Context;

//Se agrega la función de callback para recibir esos eventos
...
static void run(Context* context) {
    ...
    while(is_running) {
        ...
        if(status == FuriStatusOk && event.type == InputTypeLong && event.key == InputKeyBack) {
            is_running = false;
        }
    }
}

//Libera los recursos solicitados al sistema
